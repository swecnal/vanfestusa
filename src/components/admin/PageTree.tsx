...
interface TreeNode {
  ...
  children: TreeNode[];
  expanded: boolean;
  parentSlug: string; // parent path for ordering
}
...
function buildTree(pages: PageItem[], orderMap: Record<string, string[]>): TreeNode {
  ...
  // Apply stored order to each level
  ...
  return (
    <div ref={setNodeRef} style={style}>
      ...
      >
        ...
        {depth === 0 ? (
          <svg
            className="w-4 h-4 flex-shrink-0 text-teal"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={1.5}
...
export default function PageTree({ collapsed }: { collapsed: boolean }) {
  ...
  // Load page order from global_settings
  ...
  const handleAddChild = async (parentSlug: string) => {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Find which sibling group both belong to
    ...
    // They must share a parent for reordering
