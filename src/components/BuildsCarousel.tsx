...
export default function BuildsCarousel() {
  ...
  const next = useCallback(() => {
    goToSlide((current + 1) % builds.length);
  }, [current, goToSlide]);

  const prev = useCallback(() => {
    goToSlide((current - 1 + builds.length) % builds.length);
  }, [current, goToSlide]);
  ...
  // We render a sliding window of images
  const getVisibleIndices = (count: number) => {
    const indices: number[] = [];
    for (let i = 0; i < count; i++) {
      indices.push((current + i) % builds.length);
    }
    return indices;
  };

  const visible = getVisibleIndices(4);
