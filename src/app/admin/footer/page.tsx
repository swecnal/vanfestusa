...
export default function FooterEditorPage() {
  ...
  // Scroll iframe to bottom when loaded so footer is visible
  const handleIframeLoad = () => {
    try {
      const iframe = iframeRef.current;
      if (iframe?.contentWindow) {
        iframe.contentWindow.scrollTo({ top: iframe.contentDocument?.body.scrollHeight || 99999, behavior: "smooth" });
      }
    } catch {
      // cross-origin, ignore
    }
    ...
    <div className="flex gap-0 -m-3 md:-m-6 h-[calc(100vh-57px)]">
      ...
      <div className="flex-1 min-w-0 flex flex-col bg-gray-100">
        ...
        <div className="flex-shrink-0 bg-white border-b border-gray-200 px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h3 className="font-display font-bold text-sm text-charcoal">Footer Editor</h3>
      ...
      <div className="w-96 flex-shrink-0 bg-white border-l border-gray-200 flex flex-col h-full">
        <div className="flex-shrink-0 bg-white p-4 border-b border-gray-100">
          <h3 className="font-display font-semibold text-sm text-charcoal">Footer Builder</h3>
          <p className="text-[10px] text-gray-400 mt-0.5">Edit columns, elements, and style. Changes appear in preview after saving.</p>
        </div>
        <div className="flex-1 overflow-y-auto admin-scrollbar">
          <FooterBuilder onSave={handleSave} />
        </div>
      </div>
    </div>
  );
